...
- 2026-09-18：新增当日三篇教程解释的 5 个术语至待观察组。RAG 召回排查篇：MMR（最大边际相关性）；RAG 幻觉治理篇：FactCheckingEvaluator；技术决策日志 #03 篇：熔断（Circuit Breaker）、RBAC（基于角色的访问控制）、bucket4j。其余当日术语（Rerank、Recall@K、RRF、metadata、Faithfulness、拒答、阈值切点、结构化输出、ADR、指数退避、P95 延迟、黄金测试集、A/B 对比实验、成本红线、加权决策矩阵、撤销条件等）已在白名单。
